...
public class TravelController {
    ...
    @PostMapping("/join/{travelId}")
    public ResponseEntity<Void> joinTravel(@PathVariable UUID travelId, @RequestParam UUID studentId) {
        travelService.joinTravel(travelId, studentId);
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/leave/{travelId}")
    public ResponseEntity<Void> leaveTravel (@PathVariable UUID travelId, @RequestParam UUID studentId) {
